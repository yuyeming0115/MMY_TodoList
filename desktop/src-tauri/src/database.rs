...
impl Database {
    ...
    /// 自动保存大图片剪贴板内容（带缩略图，用于快速复制）
    /// 用于超大尺寸图片，避免内存膨胀，同时提供缩略图用于快速复制
    pub fn add_auto_clipboard_image_large_with_thumbnail(&self, width: &u32, height: &u32, png_data: &[u8], thumbnail_base64: Option<&str>) -> SqliteResult<()> {
        let id = Uuid::new_v4().to_string();
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
        ...
        // 使用 INSERT OR IGNORE 防止竞态重复
        // 缩略图存储在 image_base64 列（用于快速复制）
        let affected = conn.execute(
            "INSERT OR IGNORE INTO clipboard_items (id, category_id, title, content, image_base64, image_path, priority, sort_order, created_at, expires_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, 2, ?7, ?8, ?9)",
            rusqlite::params![&id, category_id, &title, "", thumbnail_base64, &image_path, min_order - 1, &now, expires_at],
        ).map_err(|e| {
            eprintln!("[剪贴板] 插入数据库失败: {:?}", e);
            e
        })?;
